...
export default async function AccountPage() {
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-6 px-4 py-12">
      ...
      <dl className="flex flex-col gap-3 rounded-lg border border-black/10 p-4">
        <div className="flex justify-between">
          <dt className="text-black/60">ชื่อ-นามสกุล</dt>
          <dd>{user?.fullName}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-black/60">อีเมล</dt>
          <dd>{user?.email}</dd>
        </div>
        <div className="flex justify-between">
          <dt className="text-black/60">สิทธิ์การใช้งาน</dt>
          <dd>{ROLE_LABEL[session.role] ?? session.role}</dd>
        </div>
        {user?.tenant && (
          <div className="flex justify-between">
            <dt className="text-black/60">ร้านค้า</dt>
            <dd>{user.tenant.shopName}</dd>
          </div>
        )}
        {user?.memberTier && (
          <div className="flex justify-between">
            <dt className="text-black/60">ระดับสมาชิก</dt>
      ...
      <Link
        ...
        className="w-fit rounded-md border border-black/20 px-4 py-2"
      >
        จัดการที่อยู่จัดส่ง
      </Link>
