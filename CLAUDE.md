    ...
    **ระบบผู้ขายหลายราย (marketplace) ที่ทำไว้แล้ว** (roadmap ข้อ 8, เสร็จแล้ว 2026-08-12, ดู [docs/marketplace-vendors.md](./docs/marketplace-vendors.md) ก่อนแก้ไขส่วนนี้เสมอ):
    ...
    - Vendor **เพิ่มสินค้าเองได้** ที่ `/vendor/products` (มีปุ่มเพิ่มสินค้าใหม่ที่ `/vendor/products/new`) และ**เห็น/จัดการได้เฉพาะสินค้าของตัวเองเท่านั้น** — ทั้งหน้า list (`src/app/vendor/products/page.tsx` query ด้วย `where: { vendorId: session.vendorId }`) และทุก action แก้/ลบ (`src/lib/vendor-products/actions.ts` เช็ค `vendorId` ซ้ำอีกชั้นกันไม่ให้แก้/เดา productId ของ vendor อื่น) ฟอร์มสินค้ารองรับรูปเดียวผ่าน URL เท่านั้น (ยังไม่มีอัปโหลดไฟล์/รูปใบรับประกันแยกเหมือนสินค้าที่ seed ไว้)
    ...
    8. ✅ ระบบผู้ขายหลายราย — เลือก implement เป็น "หลายผู้ขายในร้านเดียว" (marketplace ภายใน tenant เดียว คล้าย Shopee/Lazada) ตามที่ผู้ใช้ระบุ — ดูรายละเอียดที่ [docs/marketplace-vendors.md](./docs/marketplace-vendors.md) (**มีแผนเสริมที่ผู้ใช้ขอไว้แต่ยังไม่ได้ทำ** — แจ้งเตือน vendor เมื่อมีออร์เดอร์ใหม่, หน้าแอดมินสรุป/ส่งรายการสั่งซื้อให้ vendor, รหัสสินค้า (SKU), เลขพัสดุ/tracking, ระบบ payout/commission, vendor เปลี่ยนรหัสผ่านเอง — ดูหัวข้อ "แผนที่จะเพิ่ม" ใน docs/marketplace-vendors.md)
